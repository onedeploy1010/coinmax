...
export const OverviewTab: React.FC<Props> = ({ rows }) => {
  ...
  const cards: { label: string; value: string; color?: string }[] = [
    ...
    { label: "总回购 MX", value: fmt(last.total_mx_buyback), color: "#06b6d4" },
    { label: "总 MX 兑付", value: fmt(last.total_mx_redemptions), color: "#8b5cf6" },
    { label: "总 MX 销毁", value: fmt(last.total_mx_burned), color: "#f97316" },
    { label: "净卖压 MX", value: fmt(last.total_mx_sold - last.total_mx_buyback), color: last.total_mx_sold > last.total_mx_buyback ? "#ef4444" : "#22c55e" },
    ...
    { label: "总 MX 发行", value: fmt(last.total_mx_emitted) },
    { label: "总 MX 延迟释放", value: fmt(last.total_mx_deferred) },
