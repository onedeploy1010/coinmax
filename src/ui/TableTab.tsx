...
const COLUMNS: { key: keyof DailyRow; label: string; fmt?: (v: number | boolean) => string }[] = [
  ...
  { key: "payout_ar_today", label: "MX发行", fmt: (v) => (v as number).toFixed(2) },
  ...
  { key: "sold_ar_today", label: "售MX", fmt: (v) => (v as number).toFixed(2) },
  ...
  { key: "ar_buyback_out", label: "回购MX", fmt: (v) => (v as number).toFixed(2) },
